 ...
 * CONFIG.remoteBases 的第一项是 "./"，也就是**同源优先**：资源跟 index.html
 * 从同一个站点取（相对路径按「页面地址」解析，不是按 loader.js 的位置），
 * 所以整站丢到任意服务器上都能跑，传完文件刷新就是新版，不用重新打包。
 * 页面里不带任何本地副本，也不做「悄悄退回旧文件」那种兜底：
 * 全都拿不到就把原因摆到页面上，别让人对着白屏猜。
 *
 *     同源  js/after.js
 *     兜底  https://decadeguo.github.io/out_link/js/after.js
 *
 * 目录结构（相对位置不能变，index.html 和 js/ css/ imgs/ 同级）：
 *     index.html
 *     js/render_html.js
 *     js/words-loader.js
 *     js/after.js
 ...
 *      远端页面 fetch 会被跨域拦掉。
 *   2. 资源 URL 一律拼 ?_t=时间戳：静态服务器通常只回 Last-Modified，
 *      没有 Cache-Control / ETag，浏览器会一直复用旧脚本，改了也不生效。
 *   3. 地址按顺序试，第一个成功的就**锁定**，后面的文件都从它取。
 *      不锁定的话可能出现「结构来自 A 站、逻辑来自 B 站的旧版本」这种错位页面。
 *      锁定的地址挂了也不换站，直接报错把整页停下，绝不拼一个半新半旧的页面。
 *   4. 熔断：某个脚本所有地址都试完还是不通，本次加载剩下的资源直接失败收场，
 ...
 *   7. index.html 里 loader.js 用相对路径 ./js/loader.js（同源），
 *      所以只有 loader.js 必须和 index.html 放在一起，其余资源由它自己去找。
 * ---------------------------------------------------------------------------
 */
(function (global) {
    "use strict";
    ...
    /* ========================= 改这里 ========================= */
    var CONFIG = {
        // 资源根目录，**按顺序**试，第一个通的锁定使用。
        // "./" = 跟 index.html 同一个站（相对「页面地址」解析，不是相对 loader.js），
        // 同源拿不到（例如本地壳直接打开、本地没对应文件）才退到下面两个远端。
        // 加备用站就往后面追加；换服务器不用改这里。
        remoteBases: [
            "./",
